...
在 `@路径` 后加 `#L` 后缀可只附加文件的指定行区间（1-based、含端点）：

- `@src/a.ts#L12` — 只附加第 12 行；`@src/a.ts#L12-14` — 附加第 12–14 行。
- 也兼容官方无 `L` 写法：`@src/a.ts#12-14`。
- 含空格的路径用引号形式：`@"my dir/a.ts"#L3-5`。
- 区间起点超出文件总行数时**回退为附加整个文件**并弹出黄条提示——显式意图
  不会被静默丢弃。极少数以 `#L数字` 结尾的真实路径请用引号形式书写。
