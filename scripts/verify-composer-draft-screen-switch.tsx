...
const SCREEN_MARK = {
  settings: '没有可配置的插件设置',
  // /resume and /agentview are one screen now (SessionSupervisor, #890) and
  // both land on its title; the per-command empty-state lines are gone.
  resume: '会话管理',
  tree: '正在加载会话树',
  jobs: '当前会话暂无后台任务',
  agentview: '会话管理',
...
/**
 * Caret column expressed as an offset INTO the draft, not as an absolute
 * column: the row's leading affordances (the ⌂ session entry, the ❯ prompt,
 * a vim badge) shift the absolute column whenever the layout gains or loses
 * one, and a pinned absolute column silently turns every "caret parked at
 * offset N" wait into a timeout.
 */
function caretDraftCol(app: Harness): number {
  return cursorPos(app).x - (promptRow(app).indexOf('❯') + 2)
}
...
async function scenarioCtrlA(): Promise<string> {
  ...
  try {
    ...
    await waitFor(scenario, 'caret parked mid-draft (offset 5)', () => caretDraftCol(app) === 5)
...
async function scenarioCtrlT(): Promise<string> {
  ...
  try {
    ...
    await waitFor(
      ...
      () => caretDraftCol(app) === 5 && cursorPos(app).y === range.top + 2,
    )
...
async function scenarioRoutedScreens(): Promise<string> {
  ...
  try {
    ...
    await waitFor(scenario, 'caret parked at offset 7', () => caretDraftCol(app) === 7)
